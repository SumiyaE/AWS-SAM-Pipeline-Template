...
```bash
pipeLineTest$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
pipeLineTest$ sam local start-api
pipeLineTest$ curl http://localhost:3000/
```
...
```bash
pipeLineTest$ sam logs -n HelloWorldFunction --stack-name pipeLineTest --tail
```
...
```bash
aws cloudformation delete-stack --stack-name pipeLineTest
```
